...
read = '~/Data'
modelWrite = 'Models/with'
resultWrite = 'Results/with'
...
trainset = torchvision.datasets.ImageFolder(
    root=train_data_dir, transform=transform)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                          shuffle=True, num_workers=2)

valset = torchvision.datasets.ImageFolder(
    root=val_data_dir, transform=transform)
...
pd.DataFrame(columns=['epoch', 'aeroplane', 'cat', 'deer',
                      'dog', 'frog']).to_csv(classValName, index=False)
pd.DataFrame(columns=['aeroplane', 'cat', 'deer', 'dog',
                      'frog']).to_csv(classTestName, index=False)
...
class Net(nn.Module):
    def __init__(self):
        ...
        # change out_features according to number of classes
...
def classwise_test(testloader, model, isVal, epoch):
    ...
    if isVal:
        with open(classValName, 'a') as newFile:
            newFileWriter = csv.writer(newFile)
            newFileWriter.writerow([epoch+1, 100 * class_correct[0] / class_total[0], 100 * class_correct[1] / class_total[1], 100 *
                                    class_correct[2] / class_total[2], 100 * class_correct[3] / class_total[3], 100 * class_correct[4] / class_total[4]])
    else:
        with open(classTestName, 'a') as newFile:
            newFileWriter = csv.writer(newFile)
            newFileWriter.writerow([100 * class_correct[0] / class_total[0], 100 * class_correct[1] / class_total[1], 100 *
                                    class_correct[2] / class_total[2], 100 * class_correct[3] / class_total[3], 100 * class_correct[4] / class_total[4]])
